""" copy of service_api.py from ion-ux project

to be used as the service gateway client for ape tests

only change: instead of importing config.GATEWAY_*,
these two literals are defined in this file with defaults.
derived URLs are now created dynamically so host and port can be changed on the fly.
"""
...
def get_service_url():
    return 'http://%s:%d/ion-service' % (GATEWAY_HOST, GATEWAY_PORT)
def get_agent_url():
...
class ServiceApi(object):
    ...
    def request_enrollment_in_org(marine_facility_id, user_id):
        ...
        url = '%s/org_management/request_enroll' % get_service_url()
...
def build_agent_request(agent_id, operation_name, params={}):
    url = '%s/%s/%s' % (get_agent_url(), agent_id, operation_name)
